import React, { useState, useMemo } from "react";
import { useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import Icon from "@/components/ui/icon";

const Services = () => {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState("");
  const [activeCategory, setActiveCategory] = useState<string | null>(null);
  const [showContactForm, setShowContactForm] = useState(false);
  const [formData, setFormData] = useState({
    name: "",
    phone: "",
    problem: "",
  });

  // РАСШИРЕННЫЕ КАТЕГОРИИ ПРОБЛЕМ
  const problemCategories = [
    {
      id: "money",
      title: "💸 Финансы и долги",
      icon: "CreditCard",
      color: "bg-gradient-to-br from-red-50 to-orange-50",
      border: "border-red-100",
      keywords: [
        "долги",
        "кредиты",
        "займы",
        "микрофинансы",
        "коллекторы",
        "банкротство",
        "взыскание",
        "задолженность",
        "неплатеж",
        "должник",
        "проценты",
        "просрочка",
        "финансовые споры",
        "денежные обязательства",
        "списание долгов",
        "реструктуризация",
        "кредитная история",
        "финансовый управляющий",
      ],
    },
    {
      id: "housing",
      title: "🏠 Недвижимость",
      icon: "Home",
      color: "bg-gradient-to-br from-blue-50 to-cyan-50",
      border: "border-blue-100",
      keywords: [
        "квартира",
        "дом",
        "жилье",
        "потоп",
        "залив",
        "соседи",
        "ремонт",
        "ущерб",
        "аренда",
        "ипотека",
        "сделка",
        "перепланировка",
        "приватизация",
        "выселение",
        "жилищный спор",
        "коммунальные услуги",
        "капремонт",
        "ТСЖ",
      ],
    },
    {
      id: "family",
      title: "👨‍👩‍👦 Семейное право",
      icon: "Users",
      color: "bg-gradient-to-br from-pink-50 to-rose-50",
      border: "border-pink-100",
      keywords: [
        "развод",
        "брак",
        "алименты",
        "дети",
        "имущество",
        "наследство",
        "завещание",
        "опека",
        "родительские права",
        "совместная собственность",
        "брачный договор",
        "раздел имущества",
        "семейные споры",
        "определение порядка общения",
        "лишение родительских прав",
        "усыновление",
        "установление отцовства",
      ],
    },
    {
      id: "auto",
      title: "🚗 Автомобильное право",
      icon: "Car",
      color: "bg-gradient-to-br from-green-50 to-emerald-50",
      border: "border-green-100",
      keywords: [
        "дтп",
        "авария",
        "страховая",
        "осаго",
        "каско",
        "авто",
        "машина",
        "ущерб",
        "ремонт",
        "штрафы",
        "лишение прав",
        "европротокол",
        "страховая выплата",
        "виновник дтп",
        "потерпевший",
        "гибдд",
        "административные правонарушения",
        "алкогольное опьянение",
        "техосмотр",
        "купля-продажа авто",
        "некачественный ремонт",
        "автосервис",
        "гарантия на ремонт",
      ],
    },
    {
      id: "work",
      title: "💼 Трудовое право",
      icon: "Briefcase",
      color: "bg-gradient-to-br from-yellow-50 to-amber-50",
      border: "border-yellow-100",
      keywords: [
        "зарплата",
        "увольнение",
        "работодатель",
        "отпуск",
        "больничный",
        "трудовой договор",
        "сокращение",
        "дисциплинарное взыскание",
        "незаконное увольнение",
        "задержка зарплаты",
        "трудовые права",
        "компенсация",
        "восстановление на работе",
        "рабочее место",
        "трудовая инспекция",
        "командировка",
        "материальная ответственность",
        "профсоюз",
      ],
    },
    {
      id: "consumer",
      title: "🛍️ Защита прав потребителей",
      icon: "ShoppingCart",
      color: "bg-gradient-to-br from-purple-50 to-violet-50",
      border: "border-purple-100",
      keywords: [
        // Основные понятия
        "защита прав потребителей",
        "закон о защите прав потребителей",
        "права потребителя",
        "претензия продавцу",
        "неустойка",
        "штраф 50%",
        "существенный недостаток",
        "гарантийный срок",
        "срок службы",
        "ненадлежащее качество",
        "ненадлежащая информация",

        // Товары
        "товар ненадлежащего качества",
        "бракованный товар",
        "технически сложный товар",
        "возврат товара",
        "обмен товара",
        "ремонт по гарантии",
        "гарантийный ремонт",
        "возврат денег за товар",
        "некачественный товар",
        "не соответствует описанию",
        "не подошел товар",

        // Услуги
        "некачественная услуга",
        "услуга ненадлежащего качества",
        "нарушение сроков оказания услуг",
        "договор оказания услуг",
        "смета",
        "некачественный ремонт",
        "строительные услуги",
        "ремонтные работы",

        // Специфичные сферы
        // Автосервис
        "некачественный ремонт автомобиля",
        "автосервис обманул",
        "гараж сделал плохой ремонт",
        "сто испортил машину",
        "возврат автомобиля после ремонта",
        "гарантия на ремонт авто",
        "некачественные запчасти",
        "диагностика авто",

        // Возврат авто
        "возврат автомобиля",
        "возврат машины",
        "отказ от договора купли-продажи авто",
        "обман при покупке автомобиля",
        "скрытые дефекты автомобиля",
        "авто после дтп",
        "скрученный пробег",
        "юридически чистый автомобиль",
        "автосалон обманул",
        "дилер нарушил гарантию",

        // Магазины и продавцы
        "магазин обманул",
        "интернет магазин обманул",
        "не доставили товар",
        "подделка товара",
        "фальсификат",
        "неправильная цена",
        "акция обман",
        "распродажа бракованного товара",
        "продажа просроченного товара",
        "продукты просрочка",

        // Услуги разные
        "туроператор обманул",
        "отмена тура",
        "недобросовестный туроператор",
        "фитнес клуб обманул",
        "невозможно расторгнуть договор с фитнес клубом",
        "образовательные курсы некачественные",
        "онлайн обучение обман",
        "ремонт квартиры некачественный",
        "строительная бригада обманула",
        "подрядчик не соблюдает сроки",
        "некачественные строительные материалы",
        "услуги жкх некачественные",
        "неправильное начисление коммунальных платежей",

        // Финансовые услуги
        "навязанная страховка",
        "кредит с навязанной страховкой",
        "банк навязал услугу",
        "страховка при кредите",
        "отказ от страховки",
        "возврат страховой премии",
        "скрытые комиссии",
        "незаконные проценты",

        // Цифровые товары
        "цифровой товар не работает",
        "программа не соответствует описанию",
        "игра не запускается",
        "подписка навязана",
        "автопродление подписки",
        "отказ в возврате денег за цифровой товар",
        "заблокировали аккаунт",

        // Общие ситуации
        "обвес обмер обсчет",
        "введение в заблуждение",
        "неправомерный отказ",
        "отказ в гарантийном обслуживании",
        "не выдают чек",
        "нет товарного чека",
        "возврат без чека",
        "подарок ненадлежащего качества",
        "расторжение договора",
        "досудебная претензия",
        "судебный иск потребителя",
        "общество защиты прав потребителей",
        "роспотребнадзор",
        "жалоба в роспотребнадзор",
      ],
    },
    {
      id: "land",
      title: "🌳 Земельное право",
      icon: "TreePine",
      color: "bg-gradient-to-br from-emerald-50 to-green-50",
      border: "border-emerald-100",
      keywords: [
        "земельный участок",
        "межевание",
        "кадастр",
        "аренда земли",
        "собственность",
        "границы",
        "садоводство",
        "ИЖС",
        "сельхозназначение",
        "сервитут",
        "изъятие",
        "дачный участок",
        "оформление земли",
        "споры с соседями",
        "прирезка",
        "кадастровая ошибка",
        "разрешение на строительство",
      ],
    },
    {
      id: "migration",
      title: "🛂 Миграционное право",
      icon: "Globe",
      color: "bg-gradient-to-br from-sky-50 to-blue-50",
      border: "border-sky-100",
      keywords: [
        "гражданство",
        "вид на жительство",
        "внж",
        "разрешение на временное проживание",
        "рвп",
        "патент",
        "миграционный учет",
        "депортация",
        "запрет на въезд",
        "иностранный гражданин",
        "разрешение на работу",
        "миграционная карта",
        "загранпаспорт",
        "восстановление документов",
        "беженство",
      ],
    },
    {
      id: "construction",
      title: "🏗️ Строительное право",
      icon: "Building",
      color: "bg-gradient-to-br from-amber-50 to-orange-50",
      border: "border-amber-100",
      keywords: [
        "застройщик",
        "дду",
        "подрядчик",
        "строительные работы",
        "смета",
        "некачественный ремонт",
        "сроки строительства",
        "приемка объекта",
        "строительная экспертиза",
        "технический надзор",
        "разрешительная документация",
        "генеральный подрядчик",
        "субподряд",
        "строительные нормы",
        "реконструкция",
        "капитальный ремонт",
      ],
    },
  ];

  // ПОЛНЫЙ СПИСОК ЮРИДИЧЕСКИХ УСЛУГ
  const legalServices = [
    // === Финансы и долги ===
    {
      problem: "Банкротство физических лиц",
      description: "Спишем долги по кредитам и займам законно",
      solution: "Процедура банкротства",
      result: "Списание до 100% долгов, защита от коллекторов",
      time: "5-9 месяцев",
      stats: ["Опыт 12+ лет", "97% успешных дел", "Списали до 15 млн ₽"],
      link: "/bankruptcy-lawyer",
      icon: "FileCheck",
      color: "from-red-500 to-orange-500",
      categories: ["money"],
      keywords: [
        "не могу платить кредиты",
        "долги банкам",
        "коллекторы звонят",
        "микрофинансы",
        "финансовая несостоятельность",
        "освобождение от долгов",
        "реструктуризация долгов",
        "процедура банкротства",
        "списание кредитов",
        "судебное банкротство",
        "арбитражный управляющий",
      ],
    },
    {
      problem: "Взыскание задолженности",
      description: "Вернем ваши деньги с должника через суд",
      solution: "Судебное взыскание",
      result: "Возврат всей суммы долга + проценты и штрафы",
      time: "2-4 месяца",
      stats: ["Опыт 15+ лет", "89% успешных взысканий", "Вернули до 23 млн ₽"],
      link: "/debt-collection",
      icon: "Scale",
      color: "from-blue-500 to-cyan-500",
      categories: ["money"],
      keywords: [
        "должник не возвращает деньги",
        "заем не отдают",
        "задолженность по договору",
        "неплатеж по расписке",
        "вернуть долг",
        "судебное взыскание",
        "принудительное исполнение",
        "исполнительный лист",
        "взыскание дебиторки",
        "денежные обязательства",
        "договор займа",
      ],
    },

    // === Недвижимость ===
    {
      problem: "Споры с застройщиком",
      description: "Решим проблемы с новостройкой: дефекты, просрочка, обман",
      solution: "Претензионная работа и суд",
      result: "Устранение недостатков или возврат денег + неустойка",
      time: "4-8 месяцев",
      stats: ["Опыт 10+ лет", "92% выигранных дел", "Вернули до 5 млн ₽"],
      link: "/disputes-with-developers",
      icon: "Building",
      color: "from-indigo-500 to-purple-500",
      categories: ["housing", "construction"],
      keywords: [
        "застройщик нарушил сроки",
        "квартира с недостатками",
        "нарушение договора дду",
        "некачественная отделка",
        "скрытые дефекты",
        "отказ от договора",
        "взыскание неустойки",
        "расторжение договора дду",
        "дольщик",
        "незавершенное строительство",
        "строительные дефекты",
      ],
    },
    {
      problem: "Возмещение ущерба от залива",
      description: "Взыщем деньги на ремонт, если вас затопили соседи",
      solution: "Досудебное урегулирование и суд",
      result: "Полная компенсация ущерба + моральный вред",
      time: "2-4 месяца",
      stats: ["Опыт 8+ лет", "95% успешных дел", "Взыскали до 2 млн ₽"],
      link: "/flood-damage",
      icon: "Droplets",
      color: "from-cyan-500 to-blue-500",
      categories: ["housing"],
      keywords: [
        "затопили квартиру",
        "потоп от соседей",
        "ремонт после залива",
        "ущерб имуществу",
        "компенсация ущерба",
        "оценка ущерба",
        "соседи сверху залили",
        "залив квартиры",
        "затопление",
        "водопровод прорвало",
        "ущерб от соседей",
      ],
    },

    // === Семейное право ===
    {
      problem: "Расторжение брака и раздел имущества",
      description: "Поможем при разводе справедливо разделить имущество",
      solution: "Мировое соглашение или суд",
      result: "Справедливый раздел, определение долей, сохранение имущества",
      time: "3-6 месяцев",
      stats: ["Опыт 14+ лет", "94% успешных дел", "Сохранили имущество до 87%"],
      link: "/family-lawyer",
      icon: "HeartBreak",
      color: "from-pink-500 to-rose-500",
      categories: ["family"],
      keywords: [
        "развод с мужем/женой",
        "раздел квартиры при разводе",
        "совместно нажитое имущество",
        "брачный договор",
        "определение долей",
        "спор об имуществе",
        "супружеские доли",
        "раздел ипотечной квартиры",
        "расторжение брака через суд",
        "имущественные споры супругов",
        "развод с детьми",
      ],
    },
    {
      problem: "Взыскание алиментов",
      description: "Добьемся выплат на содержание детей",
      solution: "Судебный приказ или иск",
      result: "Регулярные выплаты + взыскание задолженности + индексация",
      time: "1-2 месяца",
      stats: ["Опыт 12+ лет", "98% успешных взысканий", "Взыскали до 15 млн ₽"],
      link: "/family-lawyer",
      icon: "Baby",
      color: "from-rose-500 to-pink-500",
      categories: ["family"],
      keywords: [
        "алименты на ребенка",
        "неплательщик алиментов",
        "увеличение алиментов",
        "задолженность по алиментам",
        "взыскание алиментов в твердой сумме",
        "уклонение от алиментов",
        "лишение родительских прав",
        "изменение размера алиментов",
        "алименты на супруга",
        "исполнительное производство по алиментам",
        "содержание детей",
      ],
    },

    // === Автомобильное право (ОБНОВЛЕНО: удалена дублирующая карточка) ===
    {
      problem: "Установление вины за ДТП",
      description:
        "Доказательство невиновности и защита ваших интересов в споре о виновнике",
      solution: "Доказать невиновность",
      result: "Снятие обвинений или смягчение ответственности",
      time: "1-3 месяца",
      stats: ["Опыт 15+ лет", "98% выигранных дел", "Бесплатная консультация"],
      link: "/dtp-lawyer",
      icon: "Car",
      color: "from-green-500 to-emerald-500",
      categories: ["auto"],
      keywords: [
        "дтп вина",
        "невиновность в дтп",
        "спор о виновнике дтп",
        "установление виновника аварии",
        "административное расследование дтп",
        "экспертиза дтп",
        "обжалование постановления",
        "виновность в аварии",
        "доказательства невиновности",
        "экспертиза обстоятельств дтп",
      ],
    },
    {
      problem: "Споры со страховой",
      description: "Страховая занижает выплату или отказывает в возмещении",
      solution: "Узнать цену вопроса",
      result: "Взыскание полной суммы ущерба с страховой компании + штраф 50%",
      time: "2-4 месяца",
      stats: ["Опыт 15+ лет", "98% выигранных дел", "Бесплатная консультация"],
      link: "/dtp-lawyer",
      icon: "ShieldAlert",
      color: "from-emerald-500 to-green-500",
      categories: ["auto"],
      keywords: [
        "страховая отказывает в выплате",
        "недостаточная выплата по осаго",
        "отказ страховой компании",
        "занижение выплаты",
        "экспертиза ущерба",
        "досудебная претензия страховой",
        "судебный спор со страховой",
        "взыскание страхового возмещения",
        "каско выплата",
        "неправомерный отказ страховой",
      ],
    },
    {
      problem: "Взыскание ущерба от ДТП",
      description: "Виновник не платит за ремонт или вред здоровью",
      solution: "Бесплатная консультация",
      result: "Полное возмещение ущерба здоровью и имуществу + моральный вред",
      time: "3-5 месяцев",
      stats: ["Опыт 15+ лет", "98% выигранных дел", "Бесплатная консультация"],
      link: "/dtp-lawyer",
      icon: "FileWarning",
      color: "from-teal-500 to-green-500",
      categories: ["auto"],
      keywords: [
        "взыскание ущерба с виновника дтп",
        "ущерб здоровью в дтп",
        "ремонт автомобиля после аварии",
        "компенсация вреда здоровью",
        "моральный вред дтп",
        "иск к виновнику дтп",
        "возмещение ущерба имуществу",
        "лечение после аварии",
        "утрата трудоспособности дтп",
        "упущенная выгода дтп",
      ],
    },
    {
      problem: "Лишают прав за алкоголь",
      description: "Обжалование протокола и защита в суде",
      solution: "Оценить мои шансы",
      result:
        "Сохранение водительского удостоверения или сокращение срока лишения",
      time: "1-2 месяца",
      stats: ["Опыт 15+ лет", "98% выигранных дел", "Бесплатная консультация"],
      link: "/dtp-lawyer",
      icon: "Wine",
      color: "from-amber-500 to-orange-500",
      categories: ["auto"],
      keywords: [
        "лишение прав за алкоголь",
        "алкогольное опьянение за рулем",
        "обжалование лишения прав",
        "административный протокол",
        "медицинское освидетельствование",
        "возврат прав досрочно",
        "суд по лишению прав",
        "административное правонарушение",
        "отказ от медосвидетельствования",
        "управление в состоянии опьянения",
      ],
    },
    {
      problem: "Незаконный штраф ГИБДД",
      description: "Обжалование необоснованных штрафов и постановлений",
      solution: "Узнать, как решить",
      result: "Отмена незаконного штрафа, возврат денег, сохранение прав",
      time: "1-2 месяца",
      stats: ["Опыт 15+ лет", "98% выигранных дел", "Бесплатная консультация"],
      link: "/dtp-lawyer",
      icon: "FileText",
      color: "from-yellow-500 to-amber-500",
      categories: ["auto"],
      keywords: [
        "незаконный штраф гибдд",
        "обжалование штрафа",
        "необоснованное постановление",
        "ошибочный штраф",
        "протокол гибдд",
        "административная жалоба",
        "отмена постановления",
        "нарушение пдд",
        "автофиксация нарушений",
        "камера штраф",
      ],
    },
    // Карточка "СТО сделала плохой ремонт" удалена - заменена на улучшенную версию в разделе потребителей

    // === Трудовое право ===
    {
      problem: "Взыскание заработной платы",
      description: "Вернем невыплаченную зарплату и компенсацию",
      solution: "Трудовая инспекция и суд",
      result:
        "Взыскание всей задолженности + 1/150 ключевой ставки за каждый день",
      time: "1-2 месяца",
      stats: ["Опыт 10+ лет", "100% успешных дел", "Вернули до 8 млн ₽"],
      link: "/labor-law",
      icon: "Banknote",
      color: "from-amber-500 to-yellow-500",
      categories: ["work"],
      keywords: [
        "не выплачивают зарплату",
        "задержка заработной платы",
        "неполная выплата",
        "расчет при увольнении",
        "компенсация за задержку",
        "задолженность по зарплате",
        "трудовые права",
        "нарушение трудового договора",
        "неофициальная зарплата",
        "премии и надбавки",
      ],
    },
    {
      problem: "Восстановление на работе",
      description: "Вернем на работу при незаконном увольнении",
      solution: "Судебное восстановление",
      result: "Восстановление в должности + компенсация вынужденного прогула",
      time: "2-3 месяца",
      stats: [
        "Опыт 12+ лет",
        "96% успешных восстановлений",
        "Взыскали до 2.5 млн ₽",
      ],
      link: "/labor-law",
      icon: "Briefcase",
      color: "from-yellow-500 to-amber-500",
      categories: ["work"],
      keywords: [
        "незаконное увольнение",
        "восстановление в должности",
        "сокращение штата",
        "дисциплинарное взыскание",
        "расторжение трудового договора",
        "нарушение процедуры увольнения",
        "компенсация за вынужденный прогул",
        "незаконное сокращение",
        "увольнение по статье",
        "трудовой спор",
      ],
    },

    // === ЗАЩИТА ПРАВ ПОТРЕБИТЕЛЕЙ (РАСШИРЕННЫЙ БЛОК) ===
    {
      problem: "Возврат автомобиля с недостатками",
      description: "Вернем деньги за автомобиль с существенными дефектами",
      solution: "Претензия продавцу и суд",
      result: "Расторжение договора + возврат полной стоимости + неустойка",
      time: "2-4 месяца",
      stats: ["Опыт 11+ лет", "93% успешных дел", "Вернули до 3.5 млн ₽"],
      link: "/consumer-protection",
      icon: "Car",
      color: "from-purple-500 to-indigo-500",
      categories: ["consumer", "auto"],
      keywords: [
        "возврат автомобиля",
        "авто с дефектами",
        "скрытые недостатки машины",
        "автосалон обманул",
        "скрученный пробег",
        "авто после дтп",
        "дилер продал бракованную машину",
        "существенные недостатки автомобиля",
        "отказ от договора купли-продажи авто",
        "гарантийный ремонт авто",
        "обман при покупке машины",
        "возврат денег за машину",
        "юридически не чистый автомобиль",
        "залоговый автомобиль",
      ],
    },
    {
      problem: "Некачественный ремонт автомобиля в СТО",
      description: "Взыскание ущерба за бракованный ремонт в автосервисе",
      solution: "Экспертиза качества и суд",
      result: "Возврат денег за ремонт + компенсация ущерба + неустойка",
      time: "2-3 месяца",
      stats: ["Опыт 9+ лет", "91% успешных дел", "Взыскали до 850 тыс. ₽"],
      link: "/consumer-protection",
      icon: "Wrench",
      color: "from-violet-500 to-purple-500",
      categories: ["consumer", "auto"],
      keywords: [
        "некачественный ремонт авто",
        "автосервис обманул",
        "гараж сделал плохой ремонт",
        "некачественные запчасти",
        "диагностика неправильная",
        "сто испортил машину",
        "возврат денег за ремонт автомобиля",
        "экспертиза качества ремонта авто",
        "неисправность после ремонта",
        "гарантия на ремонт не соблюдается",
        "неправильный ремонт двигателя",
        "брак в ремонте коробки передач",
      ],
    },
    {
      problem: "Обман в магазине или интернет-магазине",
      description: "Вернем деньги за некачественный или не доставленный товар",
      solution: "Претензия и обращение в Роспотребнадзор",
      result: "Возврат денег + неустойка 1% в день + штраф 50%",
      time: "1-2 месяца",
      stats: ["Опыт 8+ лет", "96% успешных дел", "Вернули до 1.2 млн ₽"],
      link: "/consumer-protection",
      icon: "ShoppingCart",
      color: "from-indigo-500 to-blue-500",
      categories: ["consumer"],
      keywords: [
        "магазин обманул",
        "интернет магазин не доставил товар",
        "товар не соответствует описанию",
        "подделка товара",
        "фальсификат",
        "неправильная цена",
        "акция обман",
        "распродажа бракованного товара",
        "отказ в возврате товара",
        "не принимают возврат",
        "не выдают чек",
        "обвес обмер",
        "некачественный товар в магазине",
        "просроченные продукты",
        "бракованная техника",
        "сломался телефон",
        "не работает ноутбук",
      ],
    },
    {
      problem: "Некачественные услуги (ремонт, строительство)",
      description:
        "Взыскание убытков за некачественный ремонт квартиры или дома",
      solution: "Строительная экспертиза и суд",
      result: "Возврат денег или устранение недостатков + неустойка",
      time: "3-5 месяцев",
      stats: ["Опыт 12+ лет", "90% успешных дел", "Вернули до 2.8 млн ₽"],
      link: "/consumer-protection",
      icon: "Home",
      color: "from-blue-500 to-cyan-500",
      categories: ["consumer", "construction", "housing"],
      keywords: [
        "некачественный ремонт квартиры",
        "строительная бригада обманула",
        "ремонт с дефектами",
        "некачественные строительные материалы",
        "нарушение сроков ремонта",
        "подрядчик не соблюдает договор",
        "смета превышена",
        "отделочные работы брак",
        "сантехнические работы некачественные",
        "электромонтажные работы с недостатками",
        "ремонт ванной комнаты брак",
        "косметический ремонт плохой",
      ],
    },
    {
      problem: "Проблемы с туроператором",
      description: "Вернем деньги за отмененный тур или некачественный отдых",
      solution: "Претензия туроператору и Роспотребнадзор",
      result: "Возврат полной стоимости тура + компенсация морального вреда",
      time: "2-4 месяца",
      stats: ["Опыт 7+ лет", "88% успешных дел", "Вернули до 1.5 млн ₽"],
      link: "/consumer-protection",
      icon: "Plane",
      color: "from-cyan-500 to-teal-500",
      categories: ["consumer"],
      keywords: [
        "туроператор обманул",
        "отмена тура",
        "недобросовестный туроператор",
        "отель не соответствует описанию",
        "перенос дат тура",
        "некачественное обслуживание в отеле",
        "отказ в возврате денег за тур",
        "отмена поездки по вине туроператора",
        "изменение условий договора",
        "непредоставленные услуги по туру",
        "проблемы с перелетом",
        "отсутствие трансфера",
      ],
    },
    {
      problem: "Споры с фитнес-клубом",
      description: "Расторжение договора и возврат денег за неоказанные услуги",
      solution: "Претензия и обращение в контролирующие органы",
      result: "Расторжение договора + возврат денег + компенсация",
      time: "1-3 месяца",
      stats: ["Опыт 6+ лет", "95% успешных дел", "Вернули до 650 тыс. ₽"],
      link: "/consumer-protection",
      icon: "Dumbbell",
      color: "from-teal-500 to-green-500",
      categories: ["consumer"],
      keywords: [
        "фитнес клуб обманул",
        "невозможно расторгнуть договор с фитнес клубом",
        "некачественные услуги в спортзале",
        "закрытие фитнес клуба",
        "перенос или закрытие зала",
        "не работает оборудование",
        "нет тренера",
        "изменение графика работы",
        "навязывание дополнительных услуг",
        "автопродление договора",
        "отказ в заморозке абонемента",
      ],
    },
    {
      problem: "Навязанная страховка при кредите",
      description: "Отмена навязанной страховки и возврат страховой премии",
      solution: "Претензия банку и обращение в ЦБ РФ",
      result: "Расторжение договора страхования + возврат денег",
      time: "1-2 месяца",
      stats: ["Опыт 9+ лет", "94% успешных дел", "Вернули до 450 тыс. ₽"],
      link: "/consumer-protection",
      icon: "ShieldAlert",
      color: "from-green-500 to-emerald-500",
      categories: ["consumer", "money"],
      keywords: [
        "навязанная страховка",
        "кредит с навязанной страховкой",
        "банк навязал услугу",
        "страховка при кредите",
        "отказ от страховки",
        "возврат страховой премии",
        "незаконное условие кредита",
        "принуждение к страхованию",
        "скрытые комиссии",
        "незаконные проценты",
        "потребительский кредит с навязанной услугой",
      ],
    },
    {
      problem: "Некачественные образовательные услуги",
      description: "Возврат денег за некачественные курсы или онлайн-обучение",
      solution: "Претензия образовательному центру и суд",
      result: "Расторжение договора + возврат денег + компенсация",
      time: "2-3 месяца",
      stats: ["Опыт 5+ лет", "89% успешных дел", "Вернули до 380 тыс. ₽"],
      link: "/consumer-protection",
      icon: "GraduationCap",
      color: "from-emerald-500 to-lime-500",
      categories: ["consumer"],
      keywords: [
        "образовательные курсы некачественные",
        "онлайн обучение обман",
        "не выдали сертификат",
        "преподаватель некомпетентен",
        "программа обучения не соответствует",
        "отмена занятий",
        "перенос сроков обучения",
        "некачественные материалы",
        "дистанционное обучение брак",
        "языковые курсы некачественные",
        "бухгалтерские курсы обман",
        "it курсы не соответствуют",
      ],
    },
    {
      problem: "Проблемы с цифровыми товарами",
      description: "Возврат денег за неработающие программы, игры, подписки",
      solution: "Претензия и обращение в Роспотребнадзор",
      result: "Возврат денег + компенсация ущерба",
      time: "1-2 месяца",
      stats: ["Опыт 4+ лет", "87% успешных дел", "Вернули до 250 тыс. ₽"],
      link: "/consumer-protection",
      icon: "Monitor",
      color: "from-lime-500 to-yellow-500",
      categories: ["consumer"],
      keywords: [
        "цифровой товар не работает",
        "программа не соответствует описанию",
        "игра не запускается",
        "подписка навязана",
        "автопродление подписки",
        "отказ в возврате денег за цифровой товар",
        "заблокировали аккаунт",
        "лицензия недействительна",
        "софт не работает",
        "мобильное приложение брак",
        "онлайн сервис не функционирует",
        "электронная книга с дефектами",
      ],
    },
    {
      problem: "Расторжение договора и возврат денег",
      description: "Вернем деньги за некачественный товар или услугу",
      solution: "Претензионный порядок и суд",
      result: "Возврат полной стоимости + неустойка и штраф 50%",
      time: "1-2 месяца",
      stats: ["Опыт 9+ лет", "94% успешных дел", "Вернули до 3 млн ₽"],
      link: "/consumer-protection",
      icon: "PackageCheck",
      color: "from-purple-500 to-violet-500",
      categories: ["consumer"],
      keywords: [
        "бракованный товар",
        "некачественная услуга",
        "возврат денег за товар",
        "отказ от договора",
        "существенный недостаток",
        "гарантийный ремонт",
        "претензия продавцу",
        "обман потребителя",
        "технически сложный товар",
        "закон о защите прав потребителей",
      ],
    },
    {
      problem: "Взыскание неустойки по договору",
      description: "Взыщем штрафы и пени за нарушение условий договора",
      solution: "Претензионная работа",
      result: "Взыскание неустойки в размере до 50% от суммы договора",
      time: "1-3 месяца",
      stats: ["Опыт 8+ лет", "91% успешных дел", "Взыскали до 1.5 млн ₽"],
      link: "/consumer-protection",
      icon: "FileText",
      color: "from-violet-500 to-purple-500",
      categories: ["consumer"],
      keywords: [
        "нарушение сроков",
        "неустойка по договору",
        "пени за просрочку",
        "штрафные санкции",
        "несоблюдение условий",
        "договорные обязательства",
        "взыскание пени",
        "ответственность за нарушение договора",
        "проценты за пользование чужими деньгами",
        "договор оказания услуг",
      ],
    },

    // === Земельное право ===
    {
      problem: "Споры о границах земельных участков",
      description: "Урегулируем конфликты с соседями по границам участков",
      solution: "Кадастровая экспертиза и суд",
      result: "Установление законных границ, устранение кадастровых ошибок",
      time: "3-6 месяцев",
      stats: ["Опыт 7+ лет", "90% успешных дел", "Решили более 150 споров"],
      link: "/land-law",
      icon: "Map",
      color: "from-emerald-500 to-green-500",
      categories: ["land"],
      keywords: [
        "границы участка",
        "межевание",
        "кадастровая ошибка",
        "споры с соседями",
        "нахлест границ",
        "земельный спор",
        "исправление кадастровой ошибки",
        "судебное установление границ",
        "смежный земельный участок",
      ],
    },
    {
      problem: "Оформление права собственности на землю",
      description: "Поможем оформить земельный участок в собственность",
      solution: "Сопровождение сделки и регистрация",
      result: "Получение свидетельства о праве собственности",
      time: "1-3 месяца",
      stats: [
        "Опыт 10+ лет",
        "98% успешных сделок",
        "Оформили более 300 участков",
      ],
      link: "/land-law",
      icon: "Home",
      color: "from-green-500 to-emerald-500",
      categories: ["land"],
      keywords: [
        "оформление земли в собственность",
        "приватизация земельного участка",
        "купля-продажа земли",
        "дарение земли",
        "наследование земельного участка",
        "регистрация права собственности",
        "кадастровый учет",
        "земли населенных пунктов",
        "земли сельхозназначения",
      ],
    },

    // === Миграционное право ===
    {
      problem: "Получение гражданства РФ",
      description: "Поможем оформить гражданство России",
      solution: "Подготовка документов и сопровождение",
      result: "Получение паспорта гражданина РФ",
      time: "6-12 месяцев",
      stats: [
        "Опыт 8+ лет",
        "95% успешных обращений",
        "Помогли более 200 семьям",
      ],
      link: "/migration",
      icon: "Flag",
      color: "from-blue-500 to-sky-500",
      categories: ["migration"],
      keywords: [
        "гражданство России",
        "прием в гражданство",
        "упрощенное гражданство",
        "гражданство по браку",
        "гражданство по детям",
        "нрс",
        "внж",
        "вид на жительство",
        "патент",
        "разрешение на работу",
        "миграционный учет",
      ],
    },
    {
      problem: "Оформление вида на жительство (ВНЖ)",
      description: "Поможем получить ВНЖ в России",
      solution: "Сбор документов и подача заявления",
      result: "Получение вида на жительство",
      time: "4-8 месяцев",
      stats: [
        "Опыт 6+ лет",
        "93% успешных обращений",
        "Оформили более 150 ВНЖ",
      ],
      link: "/migration",
      icon: "FileText",
      color: "from-sky-500 to-blue-500",
      categories: ["migration"],
      keywords: [
        "вид на жительство",
        "внж",
        "разрешение на временное проживание",
        "рвп",
        "миграционная карта",
        "иностранный гражданин",
        "миграционный учет",
        "продление внж",
        "внж для носителей русского языка",
      ],
    },

    // === Строительное право ===
    {
      problem: "Споры с подрядчиком по ремонту",
      description: "Взыщем убытки за некачественный ремонт квартиры или дома",
      solution: "Строительная экспертиза и суд",
      result: "Возврат денег или устранение недостатков + неустойка",
      time: "3-5 месяцев",
      stats: ["Опыт 11+ лет", "91% успешных дел", "Вернули до 4 млн ₽"],
      link: "/construction-law",
      icon: "Wrench",
      color: "from-amber-500 to-orange-500",
      categories: ["construction", "consumer", "housing"],
      keywords: [
        "некачественный ремонт",
        "строительные работы",
        "подрядчик",
        "ремонт квартиры",
        "строительная экспертиза",
        "нарушение сроков ремонта",
        "строительные дефекты",
        "взыскание ущерба",
        "договор подряда",
        "некачественные материалы",
        "защита прав потребителей",
        "жилье",
        "квартира",
      ],
    },
    {
      problem: "Технический надзор за строительством",
      description: "Контроль за качеством строительных работ",
      solution: "Поэтапная проверка и экспертиза",
      result: "Соблюдение сроков, качества и сметы строительства",
      time: "Весь период строительства",
      stats: [
        "Опыт 15+ лет",
        "100% довольных клиентов",
        "Проконтролировали 45 объектов",
      ],
      link: "/construction-law",
      icon: "Eye",
      color: "from-orange-500 to-amber-500",
      categories: ["construction"],
      keywords: [
        "технический надзор",
        "контроль строительства",
        "строительная экспертиза",
        "качество строительных работ",
        "смета строительства",
        "приемка объекта",
        "строительные нормы",
        "снип",
        "генеральный подрядчик",
        "субподрядчик",
      ],
    },
  ];

  // Улучшенный поиск с учетом пересечения категорий
  const filteredServices = useMemo(() => {
    if (!searchQuery && !activeCategory) return legalServices;

    const query = searchQuery.toLowerCase().trim();

    return legalServices.filter((service) => {
      // Фильтрация по категории - теперь проверяем массив categories
      if (activeCategory && !service.categories.includes(activeCategory)) {
        return false;
      }

      // Если есть поисковый запрос
      if (query) {
        // Ищем в основных полях и ключевых словах
        const searchIn = [
          service.problem.toLowerCase(),
          service.description.toLowerCase(),
          service.solution.toLowerCase(),
          service.result.toLowerCase(),
          ...service.keywords,
          ...service.stats.map((stat) => stat.toLowerCase()),
        ].join(" ");

        return searchIn.includes(query);
      }

      return true;
    });
  }, [searchQuery, activeCategory]);

  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Форма отправлена:", formData);
    setShowContactForm(false);
    setFormData({ name: "", phone: "", problem: "" });
    alert("Заявка отправлена! Мы свяжемся с вами в течение 15 минут.");
  };

  return (
    <div className="space-y-16">
      {/* Блок 1: Главный поиск */}
      <div className="text-center">
        <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-6">
          Юридические услуги
        </h1>
        <p className="text-xl text-gray-600 mb-10 max-w-3xl mx-auto">
          Профессиональная правовая помощь по всем отраслям права
        </p>

        {/* Поле поиска */}
        <div className="max-w-3xl mx-auto mb-8">
          <div className="relative">
            <input
              type="text"
              placeholder="Найдите нужную услугу или опишите ситуацию..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full px-6 py-5 text-lg border-2 border-gray-300 rounded-2xl focus:border-primary focus:ring-4 focus:ring-primary/10 outline-none transition-all shadow-sm"
            />
            <Icon
              name="Search"
              className="absolute right-5 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400"
            />
          </div>
        </div>

        {/* Категории */}
        <div className="mb-10">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">
            Выберите категорию проблемы:
          </h2>
          <div className="flex flex-wrap gap-3 justify-center">
            {problemCategories.map((category) => (
              <button
                key={category.id}
                onClick={() =>
                  setActiveCategory(
                    activeCategory === category.id ? null : category.id,
                  )
                }
                className={`flex items-center gap-2 px-5 py-3 rounded-xl transition-all duration-300 ${
                  activeCategory === category.id
                    ? `${category.color} ${category.border} border-2 shadow-lg scale-105`
                    : `bg-white ${category.border} border hover:shadow-md`
                }`}
              >
                <span className="text-xl">{category.title.split(" ")[0]}</span>
                <span className="font-medium">
                  {category.title.split(" ").slice(1).join(" ")}
                </span>
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* Блок 2: Результаты поиска */}
      <div>
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900">
            {activeCategory
              ? problemCategories.find((c) => c.id === activeCategory)?.title
              : "Все услуги"}
          </h2>
          {filteredServices.length > 0 && (
            <div className="text-sm text-gray-500">
              Найдено {filteredServices.length} услуг
              {filteredServices.length === 1 ? "а" : ""}
            </div>
          )}
        </div>

        {filteredServices.length === 0 ? (
          // Если ничего не найдено
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-3xl p-12 text-center border-2 border-dashed border-gray-200">
            <div className="text-6xl mb-6">🔍</div>
            <h3 className="text-2xl font-bold text-gray-800 mb-4">
              Не нашли нужную услугу?
            </h3>
            <p className="text-gray-600 mb-8 max-w-xl mx-auto">
              Опишите вашу проблему в форме ниже, и наш юрист свяжется с вами
              для бесплатной консультации.
            </p>

            <Button
              size="lg"
              onClick={() => setShowContactForm(true)}
              className="bg-gradient-to-r from-primary to-primary/90 hover:from-primary/90 hover:to-primary text-white shadow-lg px-10 py-6 text-lg"
            >
              <Icon name="MessageSquare" className="h-6 w-6 mr-3" />
              Получить бесплатную консультацию
            </Button>
          </div>
        ) : (
          // Карточки услуг
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredServices.map((service, index) => (
              <div
                key={index}
                className="group relative bg-white rounded-2xl border border-gray-200 overflow-hidden hover:border-primary/30 hover:shadow-xl transition-all duration-300 cursor-pointer h-full flex flex-col"
                onClick={() => navigate(service.link)}
              >
                {/* Цветная полоска сверху */}
                <div className={`h-2 bg-gradient-to-r ${service.color}`} />

                <div className="p-6 flex-1 flex flex-col">
                  {/* Заголовок с иконкой */}
                  <div className="flex items-start gap-4 mb-4">
                    <div
                      className={`w-12 h-12 rounded-xl bg-gradient-to-br ${service.color} flex items-center justify-center flex-shrink-0`}
                    >
                      <Icon
                        name={service.icon}
                        className="h-6 w-6 text-white"
                      />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-gray-900 leading-tight mb-1">
                        {service.problem}
                      </h3>
                      <p className="text-gray-600 text-sm mb-2">
                        {service.description}
                      </p>
                      <div className="flex items-center gap-2">
                        <div className="px-3 py-1 bg-primary/10 text-primary text-xs font-semibold rounded-full">
                          {service.solution}
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* Статистика */}
                  <div className="mb-4">
                    <div className="flex flex-wrap gap-2 mb-3">
                      {service.stats.map((stat, i) => (
                        <div
                          key={i}
                          className="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded"
                        >
                          {stat}
                        </div>
                      ))}
                    </div>
                  </div>

                  {/* Результат */}
                  <div className="mb-4 flex-1">
                    <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                      <Icon
                        name="CheckCircle"
                        className="h-4 w-4 text-green-500"
                      />
                      <span className="font-medium">Результат:</span>
                    </div>
                    <p className="text-gray-700 pl-6">{service.result}</p>
                  </div>

                  {/* Футер карточки БЕЗ ЦЕНЫ */}
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1 text-sm text-gray-500">
                        <Icon name="Clock" className="h-4 w-4" />
                        <span>{service.time}</span>
                      </div>
                    </div>

                    <div className="flex items-center text-primary font-semibold text-sm group-hover:text-primary/80">
                      <span>Подробнее</span>
                      <Icon
                        name="ArrowRight"
                        className="h-4 w-4 ml-1 transform group-hover:translate-x-1 transition-transform"
                      />
                    </div>
                  </div>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>

      {/* Блок 3: Контакты и форма */}
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl p-10 text-white">
        <div className="max-w-4xl mx-auto">
          <div className="grid md:grid-cols-2 gap-10">
            {/* Левая часть: Контакты */}
            <div>
              <h2 className="text-3xl font-bold mb-6">
                Свяжитесь с нами удобным способом
              </h2>
              <p className="text-gray-300 mb-8">
                Получите бесплатную первичную консультацию по вашему вопросу в
                течение 15 минут
              </p>

              {/* Телефон */}
              <div className="mb-8">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center">
                    <Icon name="Phone" className="h-6 w-6 text-primary" />
                  </div>
                  <div>
                    <div className="text-2xl font-black tracking-tight">
                      +7 (383) 235-95-05
                    </div>
                    <div className="text-gray-400 text-sm mt-1">
                      Новосибирск • Ежедневно с 9:00 до 21:00
                    </div>
                  </div>
                </div>
                <Button
                  size="lg"
                  onClick={() => window.open("tel:+73832359505", "_self")}
                  className="w-full bg-white text-gray-900 hover:bg-gray-100 font-bold py-6 text-lg rounded-xl shadow-lg"
                >
                  <Icon name="Phone" className="h-6 w-6 mr-3" />
                  Позвонить юристу
                </Button>
              </div>

              {/* Мессенджеры */}
              <div className="mb-6">
                <div className="text-lg font-semibold mb-4">
                  Напишите в мессенджер:
                </div>
                <div className="flex gap-4">
                  <a
                    href="https://t.me/ваш_телеграм"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1"
                  >
                    <Button className="w-full bg-[#0088cc] hover:bg-[#0077b3] text-white py-6">
                      <div className="flex items-center justify-center gap-3">
                        {/* Логотип Telegram */}
                        <svg
                          className="w-6 h-6"
                          fill="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.064-1.225-.461-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
                        </svg>
                        <span>Telegram</span>
                      </div>
                    </Button>
                  </a>
                  <a
                    href="https://max.ru"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1"
                  >
                    <Button className="w-full bg-gradient-to-r from-[#6A11CB] to-[#2575FC] hover:opacity-90 text-white py-6">
                      <div className="flex items-center justify-center gap-3">
                        {/* Иконка MAX: диалоговое облачко */}
                        <svg
                          className="w-6 h-6"
                          fill="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 11h-4v4h-2v-4H7v-2h4V7h2v4h4v2z" />
                        </svg>
                        <span>MAX</span>
                      </div>
                    </Button>
                  </a>
                </div>
              </div>

              {/* Другие контакты */}
              <div className="text-gray-400 text-sm">
                <div className="flex items-center gap-2 mb-2">
                  <Icon name="Mail" className="h-4 w-4" />
                  <span>112@юридический-сервис.рф</span>
                </div>
                <div className="flex items-center gap-2">
                  <Icon name="MapPin" className="h-4 w-4" />
                  <span>г. Новосибирск, ул. Ленина, д. 3, офис 323</span>
                </div>
              </div>
            </div>

            {/* Правая часть: Форма */}
            <div>
              <h3 className="text-2xl font-bold mb-6">
                {showContactForm
                  ? "Опишите вашу проблему"
                  : "Бесплатная консультация"}
              </h3>

              {showContactForm ? (
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Ваше имя
                    </label>
                    <input
                      type="text"
                      required
                      value={formData.name}
                      onChange={(e) =>
                        setFormData({ ...formData, name: e.target.value })
                      }
                      className="w-full px-4 py-3 bg-white/10 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                      placeholder="Иван Иванов"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Телефон
                    </label>
                    <input
                      type="tel"
                      required
                      value={formData.phone}
                      onChange={(e) =>
                        setFormData({ ...formData, phone: e.target.value })
                      }
                      className="w-full px-4 py-3 bg-white/10 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                      placeholder="+7 (999) 999-99-99"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Опишите ситуацию
                    </label>
                    <textarea
                      required
                      value={formData.problem}
                      onChange={(e) =>
                        setFormData({ ...formData, problem: e.target.value })
                      }
                      rows={4}
                      className="w-full px-4 py-3 bg-white/10 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                      placeholder="Кратко опишите вашу проблему..."
                    />
                  </div>
                  <Button
                    type="submit"
                    className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-6 text-lg rounded-xl"
                  >
                    Отправить заявку
                  </Button>
                  <p className="text-gray-400 text-xs text-center">
                    Нажимая кнопку, вы соглашаетесь с политикой
                    конфиденциальности
                  </p>
                </form>
              ) : (
                <>
                  <p className="text-gray-300 mb-8">
                    Заполните форму, и наш юрист свяжется с вами в течение 15
                    минут
                  </p>
                  <Button
                    onClick={() => setShowContactForm(true)}
                    className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-6 text-lg rounded-xl shadow-lg"
                  >
                    <Icon name="MessageSquare" className="h-6 w-6 mr-3" />
                    Получить консультацию
                  </Button>
                  <div className="mt-8 space-y-4">
                    <div className="flex items-start gap-3">
                      <Icon
                        name="CheckCircle"
                        className="h-5 w-5 text-green-400 mt-0.5"
                      />
                      <div>
                        <div className="font-semibold">Конфиденциально</div>
                        <div className="text-gray-400 text-sm">
                          Не передаём данные третьим лицам
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Icon
                        name="Clock"
                        className="h-5 w-5 text-blue-400 mt-0.5"
                      />
                      <div>
                        <div className="font-semibold">Без ожидания</div>
                        <div className="text-gray-400 text-sm">
                          Сразу соединим с профильным юристом
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <Icon
                        name="Shield"
                        className="h-5 w-5 text-yellow-400 mt-0.5"
                      />
                      <div>
                        <div className="font-semibold">Честная оценка</div>
                        <div className="text-gray-400 text-sm">
                          Скажем, если не сможем помочь
                        </div>
                      </div>
                    </div>
                  </div>
                </>
              )}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Services;
